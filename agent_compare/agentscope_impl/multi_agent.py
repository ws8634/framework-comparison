...
class AgentScopeMultiAgentScenario:
    def __init__(
        self,
    ...
    async def _planner_agent(self, task_goal: str) -> Dict[str, Any]:
        ...
        prompt = [
            {"role": "system", "content": "You are a planning agent. Create a plan with subtasks for the given goal."},
            {"role": "user", "content": f"""Given the goal: {task_goal}\nPlease create a plan with subtasks. Return a JSON object with: task_goal, subtasks, estimated_duration_minutes."""},
        ]
        response = await self.agentscope_model.ainvoke(prompt, agent_role="planner")
        ...
        plan = {
            ...
            "planner_response": response.get("content", ""),
        }
    ...
    async def _executor_agent(
        ...
        try:
            if should_fail:
                prompt = [
                    {"role": "system", "content": "You are an executor agent. Execute the task."},
                    {"role": "user", "content": f"Execute task {task_name} - this should fail due to timeout error"},
                ]
                response = await self.agentscope_model.ainvoke(
                    prompt,
                    agent_role="executor",
                )
                ...
                prompt = [
                    {"role": "system", "content": "You are an executor agent. Execute the task successfully."},
                    {"role": "user", "content": f"Execute task: {task_name} - {subtask['description']}"},
                ]
                response = await self.agentscope_model.ainvoke(
                    prompt,
                    agent_role="executor",
                ...
                result = SubTaskResult(
                    ...
                    status="completed",
                    result={
                        "execution_response": response.get("content", ""),
    ...
    async def _reviewer_agent(
        ...
        prompt = [
            {"role": "system", "content": "You are a reviewer agent. Review the execution results and provide a conclusion."},
            {"role": "user", "content": f"""Review the execution results:
...
Provide a review conclusion."""},
        ...
        response = await self.agentscope_model.ainvoke(prompt, agent_role="reviewer")
